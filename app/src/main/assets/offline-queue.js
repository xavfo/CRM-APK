(function() {
    ...
    var Q = {
            ...
            var observer = new MutationObserver(function() {
                ...
                Array.prototype.forEach.call(inputs, function(el) {
                    ...
                    if (name === 'latitude' || name === 'lat' || id === 'latitude' || id === 'lat') {
                        if (!el.value) { el.value = gps.latitude;
                            filled = true; }
                    } else if (name === 'longitude' || name === 'lng' || name === 'lon' || id === 'longitude' || id === 'lng' || id === 'lon') {
                        if (!el.value) { el.value = gps.longitude;
                            filled = true; }
                        ...
                        if (!el.value) { el.value = gps.latitude + ', ' + gps.longitude;
                            filled = true; }
                    }
                });
            });
        ...
        watchCredentialFields: function() {
            var self = this;
            var notify = function() {
                var username = null;
                var password = null;
                var fields = document.querySelectorAll('input[type="text"], input[type="email"], input[type="password"], input:not([type]), textarea');
                Array.prototype.forEach.call(fields, function(el) {
                    var name = (el.name || '').toLowerCase();
                    var id = (el.id || '').toLowerCase();
                    var value = (el.value || '').trim();
                    if (!username && (name.indexOf('user') !== -1 || name.indexOf('usuario') !== -1 || name.indexOf('email') !== -1 || name.indexOf('mail') !== -1 || name.indexOf('login') !== -1 || id.indexOf('user') !== -1 || id.indexOf('usuario') !== -1 || id.indexOf('email') !== -1 || id.indexOf('mail') !== -1 || id.indexOf('login') !== -1)) {
                        username = value;
                    }
                    if (!password && (el.type === 'password' || name.indexOf('password') !== -1 || name.indexOf('pass') !== -1 || id.indexOf('password') !== -1 || id.indexOf('pass') !== -1)) {
                        password = value;
                    }
                });
                if (username !== null || password !== null) {
                    try { AndroidBridge.onCredentialsEntered(Boolean(username || password)); } catch (e) {}
                }
            };

            document.addEventListener('input', function(e) {
                if (e.target && /input|textarea/i.test(e.target.tagName)) notify();
            }, true);
